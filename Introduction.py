...
st.markdown("""
    ...
    """)
st.warning("Il est important d'utiliser un échantillon contenant au maximum 8000 observations dans le cas d'analyse multidimensionnelle, afin d'éviter de surcharger le serveur (Plan Free). L'application stocke toutes les données en mémoire RAM pour assurer la confidentialité, celles-ci étant temporaires et supprimées à chaque rafraîchissement de page, sans enregistrement permanent sur le serveur. Cela demande également une mémoire RAM d'au moins 4 Go. De plus, une analyse d'échantillon n'exige pas de volumes de données plus importants.")
